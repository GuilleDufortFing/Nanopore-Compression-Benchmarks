...
from common.summaries import walk_and_process  # noqa: E402
...
def build_argument_parser():
    ...
    return argparse.ArgumentParser(
        description=(
            "Run the speed benchmark pipeline for one input directory, writing "
            "timestamped raw outputs and summaries for an explicit list of one "
            "or more algorithms."
        ),
        epilog=(
...
def build_algorithm_raw_dir(raw_root, algorithm, input_dir):
    return raw_root / algorithm / build_input_label(input_dir)
...
def run_pipeline(input_dir, algorithm_tokens, results_root=None, executable=None):
    ...
    benchmark_executable = resolve_named_executable("speed_benchmark", executable)
    ...
    manifest_path = write_speed_run_manifest(
        run_root,
        input_dir,
        selected_algorithms,
    ...
    for algorithm in selected_algorithms:
        ...
        summary_outputs[algorithm] = walk_and_process(
